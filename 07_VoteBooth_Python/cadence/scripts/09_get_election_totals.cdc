...
access(all) fun main(electionId: UInt64, voteboxAddress: Address?): {String: UInt}? {
    if (voteboxAddress == nil) {
        ...
        ballotTotals["totalBallotsMinted"] = voteboxRef.getElectionTotalBallotsMinted(electionId: electionId)
        ballotTotals["totalBallotsSubmitted"] = voteboxRef.getElectionTotalBallotsSubmitted(electionId: electionId)
