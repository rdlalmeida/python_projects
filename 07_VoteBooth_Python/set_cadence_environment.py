...
def main(env: str = "local") -> None:
    ...
    if (env.lower() == "local"):
        ...
        print("Switching project contracts to local mode: ")
...
def switchToLocal(source_path: Path) -> None:
    """
    Function to switch the source text provided, under the assumption that it refers to a contract, transaction, or script, with all the import statements normalised to the local environment, i.e., with the imports recognised in the local emulator
...
def switchToRemote(source_path: Path) -> None:
    """
    Function to switch the source text provided, under the assumption that it refers to a contract, transaction, or script, with all the import statements normalised to the remote environment, i.e., with the imports recognisable by the testnet or mainnet environments.
